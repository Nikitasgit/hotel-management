...
const UserDetails = (props: { params: { id: string } }) => {
  ...
  const reviewSubmitHandler = async () => {
    ...
    if (!roomId) {
      return toast.error("Please provide an ID");
    }
  ...
  // Fetch function to get user bookings
  const fetchUserBooking = async (id: string) => {
    return getUserBookings(id);
  };
  const fetchUserData = async () => {
    const { data } = await axios.get<User>("/api/users");
    return data;
  };
  // Use SWR with a key and the fetch function
  const {
    data: userBookings,
    error,
    isLoading,
  } = useSWR(userId ? `/api/userbooking/${userId}` : null, () =>
